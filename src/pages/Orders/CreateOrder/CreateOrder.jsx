...
const CreateOrder = props => {
    ...
    async function searchRecipients(q) {
        try {
            ...
            if (response.data.total > 0) setOptions(response.data.data);
            else {
                setOptions([]);
                setOpen(false);
            }
    ...
    if (props.location.state === undefined || props.location.state === null) {
        ...
        shipConfirmation =
            ...
            props.location.state.shipConfirmation === null
                ? "default"
                : props.location.state.shipConfirmation;
    ...
    return shipConfirmation === "default" ? (
        ...
        <div className={classes.Container}>
            <Button variant="contained" color="primary" onClick={goCreateRec}>
                ...
                {reci && reci.contact ? (
                    <div className={classes.Details}>
                        <div className={classes.Details__box}>
                            <p>
                                Name: <span>{reci.name}</span>
                            </p>
                        </div>
                        <div className={classes.Details__box}>
                            <p>
                                Contact: <span>{reci.contact}</span>
                            </p>
                        </div>
                        <div className={classes.Details__box}>
                            <p>
                                Phone: <span>{reci.phone}</span>
                            </p>
                        </div>
                        <div className={classes.Details__box}>
                            <p>
                                Email: <span>{reci.email}</span>
                            </p>
                        </div>
                        <div className={classes.Details__box}>
                            <p>
                                Street1: <span>{reci.street1}</span>
                            </p>
                        </div>
                        <div className={classes.Details__box}>
                            <p>
                                Street2: <span>{reci.street2}</span>
                            </p>
                        </div>
                        <div className={classes.Details__box}>
                            <p>
                                City: <span>{reci.city}</span>
                            </p>
                        </div>
                        <div className={classes.Details__box}>
                            <p>
                                State: <span>{reci.state}</span>
                            </p>
                        </div>
                        <div className={classes.Details__box}>
                            <p>
                                Zip/Postal Code: <span>{reci.postal}</span>
                            </p>
                        </div>
                        <div className={classes.Details__box}>
                            <p>
                                Country: <span>{reci.country}</span>
                            </p>
                        </div>
                        <Button
                            variant="contained"
                            color="primary"
                            onClick={() => goOrders(reci.id)}>
                            go
                        </Button>
                    </div>
                ) : null}
        ...
        <div className={classes.RecipientDetails}>
            <div>
                ...
                <div>
                    <h3>
                        Recipient: {props.location.state.order.recipient.name}{" "}
                        {props.location.state.order.recipient.contact} |{" "}
                        {props.location.state.order.recipient.country}
                    </h3>
                    <span>
                        Total Weight:{" "}
                        {Number(props.location.state.total_weight).toFixed(2)}
                    </span>
                    <br></br>
                    <span>
                        Total Packages: {props.location.state.total_packages}
                    </span>
                    <br></br>
                    <span>
                        Insurance Cost: {props.location.state.insurance_cost}{" "}
            ...
            <p>
                Your order has been successfully entered into the system and is
                pending shipment.
            </p>
            <Box width="100%" display="flex" justifyContent="left" p={0}>
                <Button
                    ...
                    onClick={() => history.push("/order/add-update")}>
        ...
        <div>
            {props.location.state.type === "pickRate" ? (
                props.location.state.data.labels.length > 0 ? (
                    ...
                    <TableContainer component={Paper}>
                        <Table aria-label="simple table">
                            ...
                            <TableBody>
                                ...
                                {props.location.state.data.labels.length > 0 ? (
                                    props.location.state.data.labels.map(
                                        function (label, i) {
                                            return (
                                                <TableRow key={i}>
                                                    <TableCell align="center">
                    ...
                    <p>No Shippment Labels</p>
                )
            ) : (
                <div>
                    <p className={classes.strong}>
                        As this order was shipped manually there are no labels
                        to print from here.
                    </p>
                </div>
            )}
