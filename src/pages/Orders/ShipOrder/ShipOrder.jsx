...
const ShipOrder = () => {
  ...
  const changeTariff = (e, editedIndex) => {
    setCustomsInfo(prev => {
      prev[editedIndex] = {
        ...prev[editedIndex],
        hs_tariff_number: e.target.value,
      };
      return prev;
    });
  };
  ...
  const renderAdditionalTextFields = () => {
    ...
    // Append additional to customs tariff.
    setCustomsTariffInputs(prev => [
      ...prev,
      <div>
        <br />
        <input
          key={`ctariff-${prev.length}`}
          type="text"
          onChange={e => changeTariff(e, parseInt(prev.length + 1))}
        />
      </div>,
    ]);
  };
              ...
              <TableRow key={rate.id}>
                ...
                <TableCell align="center">
                  ...
                  {(() => {
                    ...
                    if (
                      rate.carrier.toUpperCase() === "FEDEX" &&
                    ...
                    if (rate.carrier.toUpperCase() === "FEDEX")
                      return "3rd party billing: 210128980";
                    else return "No 3rd party billing option set";
                  })()}
                </TableCell>
                <TableCell align="center">
                  {Number(rate.list_rate) +
                    Number(location.state.order.insurance_value)}{" "}
                  {rate.list_currency}
                </TableCell>
                <TableCell align="center">
